...
**Goal:** Add an `openhance ui` subcommand that launches a local web UI with real-time WebGPU preview, video scrubbing, preset management, and FFmpeg export.
...
Detect `openhance ui` as a subcommand and delegate to a UI module.
...
export async function startUI(port: number): Promise<void> {
  console.log(`Starting openhance UI on http://localhost:${port}`);
}
```
...
function listPresets(): string[] {
  ...
  const userDir = join(homedir(), ".openhance", "presets");
...
export async function startUI(port: number): Promise<void> {
  ...
  console.log(`openhance UI running at http://localhost:${server.port}`);
...
// src/ui/app/App.tsx
export function App() {
  return <div style={{ padding: 24 }}>openhance UI — loading...</div>;
}
```
...
```json
"build": "bun run build:ui && bun build src/cli.ts --compile --outfile openhance"
```
...
Run: `bun run build && ./openhance ui`
...
test.describe("UI loads", () => {
  test("renders the app shell", async ({ page }) => {
    await page.goto("/");
    await expect(page.locator("text=openhance")).toBeVisible();
  });
...
| 2 | Subcommand routing | `openhance ui` detection |
